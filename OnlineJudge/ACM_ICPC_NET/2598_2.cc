...
void solve(int cubeNum) {
	for (int i=0; i<cubeNum-1; ++i)
		for (int s=0; s<4; ++s)
			if (code[i*4+s] == code[(cubeNum-1)*4+s]) return;
		...
		for (int bot=0; bot<6; ++bot) {
			for (int dir=0; dir<4; ++dir) {
				for (int side=0; side<4; ++side) {
					code[cubeNum*4+side] = cube[cubeNum][face[bot][(side+dir)%4]];
				}
				if (cubeNum == 3) code[16] = cube[cubeNum][face[bot][4]];
				solve(cubeNum+1);
				/*
				if (cube[cubeNum][0] == cube[cubeNum][1] && cube[cubeNum][1] == cube[cubeNum][2] &&
						cube[cubeNum][2] == cube[cubeNum][3]) return;
